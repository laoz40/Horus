...
- If RPC routes fail with `Can't resolve '@/generated/prisma/sql'`, run `pnpm db:generate` once from the repo root (TypedSQL output is gitignored; run after `pnpm install`).
