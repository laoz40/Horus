...
export function showPage(pageId) {
    ...
    const appPage = document.querySelectorAll(".page");
    ...
    if (prevActive) {
        try {
            document.dispatchEvent(new CustomEvent("page-will-hide", { detail: { pageId: prevActive.id } }));
        }
        catch (_) { /* ignore */ }
    }
    ...
    if (pageId in pageShowFunctions) {
        const pageFunction = pageShowFunctions[pageId];
        pageFunction();
    }
