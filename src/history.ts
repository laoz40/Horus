...
function buildSummaryCardHTML(workout: Workout, index: number | null = null, showDeleteButton = false) {
  ...
  const calculateExerciseVolume = (exercise: Exercise): number => {
    return exercise.sets.reduce((total, set) => 
      total + calculateSetVolume(set), 0
    );
  };
...
function editWorkout(workoutIndex: number) {
  ...
  WORKOUT_NAME_INPUT && (WORKOUT_NAME_INPUT.value = workoutToEdit.name || '');
  WORKOUT_DATE_TEXT && (WORKOUT_DATE_TEXT.textContent = formatDisplayDate(new Date(workoutToEdit.date)) || '');
