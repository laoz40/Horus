...
export function createExerciseForm(initial: ExerciseForm = { name: '', notes: '', difficulty: '', sets: [] }) {
  ...
  function addSetRowTo(setsTable: HTMLDivElement, setNumber: number, defaults: ExerciseSetForm) {
    ...
    setRow.querySelectorAll(".set-weight").forEach((input) =>
        validateNumericInput(input as HTMLInputElement, true)
      );
    // reps integer only
    setRow.querySelectorAll(".set-reps").forEach((input) =>
...
export function readExercisesFromForms() {
  ...
  return exerciseForms.map(form => {
    ...
    const exerciseNotes = ((form.querySelector('.exercise-notes') as HTMLInputElement)?.value || '').trim();
